...
let lastVideoTime = -1;
let isConnect = false;
/**@type import('ws')*/ let socket;
let mediaRecorderPose;
let modelPath = {
...
async function changeVideo(event) {
    ...
    await release();
    if (deviceId === "") {
        if (isConnect) mediaRecorder.stop();
        return;
    }
...
async function recordVideo(deviceId) {
    ...
    if (isConnect && isRecord) {
        ...
        mediaRecorderPose.ondataavailable = (event)=>{
            if (!isConnect) return;
            if (event.data && event.data.size > 0) socketPose.send(event.data);
        };
...
let isRecord = false;
let recordBtn = document.querySelector("#recordP");
...
},{}]},["icZzK","8lqZg"], "8lqZg", "parcelRequire4ef1")
